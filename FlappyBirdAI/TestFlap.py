...
def draw_windowbest(win, birds, pipes, base, score, gen, pipe_ind):
    win.blit(bg_img, (0, 0))
...
def eval_bestgenome(genomes, config):
    global GEN
    GEN += 1
    ...
    while run:
        ...
        base.move()
        draw_windowbest(win, birds, pipes, base, score, GEN, pipe_ind)
...
def run(config_file):
    ...
    if os.path.exists(model_dir + '/' + model_file):
        ...
        eval_bestgenome(genomes, config)
    else :
